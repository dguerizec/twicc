...
`git log --oneline` (most recent first): `616518aa` (consent dialog rework — question + adaptive callout) · `652b61de` ("This session" survives reloads, module cache) · `0c6f0971` (docs: proxy no-recheck + posture) · `f2dfc268` (live "Forever" eligibility + caption) · `f8967aaf` (**phase 5** — unified dedicated-page shell) · `414cf8b4` (docs) · `f42c96da` (consent) · `35163802` (origin-isolation doc) · `0fe6ed64` (phase 4 + policy) · `0b8f65e3` (3c) · `44dc128a` (3a/b) · `2ab97b3f` (2) · `1dc326a5` (1). This doc edit is the only uncommitted change. Run `git status`/`git log` for the true state.
...
- `frontend/src/components/artifacts/ArtifactBrokerPrompt.vue` — consent dialog (wa-dialog): explicit question + `Deny / Yes, forever / Yes, this session` + per-choice persistence hints + an **adaptive loopback/LAN callout** (amber/masquerade only for a domain name; neutral info otherwise; derives the bare hostname from the key). Emits `'deny'|'forever'|'session'`.
