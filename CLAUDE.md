...
| Model | Purpose |
|-------|---------|
| `Project` | Cross-provider working directory. ID derived from the directory path via `path_to_project_id`. Has `name`, `color`, `directory`, `git_root`, `total_cost`, `sessions_count`, and `worktree_of` (auto-detected self-FK to the main repository when the directory is a git worktree, `related_name="worktrees"`; never set manually). A worktree's sessions/cost/activity aggregate into its main repository. Also carries the per-project agent defaults: `default_provider` + `default_agent_settings` (per-provider bundles seeding NEW sessions; inherited up the chain — worktree main repo, then path ancestors — resolved at creation only, by `frontend/src/utils/projectAgentDefaults.js` for UI drafts and its mirror `src/twicc/project_agent_defaults.py` for CLI `create-session`; never re-resolved for a running session). |
...
**`permission_mode_if_untrusted` is NOT in the bundle.** It is a *default-shaping* field (per provider) that lives only in the global synced settings, presets, and per-project `default_agent_settings` — never on `Session`. At session creation the effective trust of the target project picks which one materializes into the single stored `permission_mode` (trusted → `permission_mode`, untrusted/unknown → `permission_mode_if_untrusted`); its allowed values are the provider's `UNTRUSTED_PERMISSION_MODES` (no `bypassPermissions`/`yolo`). The backend security floor (`core/services/trust.py` → `clamp_permission_mode_for_untrusted`, applied at agent build) re-clamps regardless of how the session was created; the CLI mirrors the same resolve+clamp at create/update time (see `cli/_drop_request/aliases.py`). Project **trust itself is a human-only decision** — never exposed as an agent-facing CLI flag or skill. See `docs/plans/2026-06-09-project-trust-design.md` §13.
