...
- **`Session`** — one JSONL file; `last_offset`/`last_line` drive incremental sync. Carries costs, `type` (session/subagent), `parent_session` (self-FK), lifecycle timestamps, the closed `AgentSettings` bundle (see below), and `layout` (JSONField, the per-session dockable-layout intention blob — full intention minus the transient `maximized`; unlike the agent bundle it stays **mutable** after creation, persisted + synced like `annotations`).
