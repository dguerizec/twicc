...
Update a project's name, color, archived state, and/or default provider — plus a `settings` sub-command for its per-provider agent-settings defaults. The directory is immutable; there is no delete (projects are archived, never removed).
- `--name TEXT` / `--unset-name`, `--color TEXT` / `--unset-color`, `--archive` / `--unarchive`, `--default-provider TEXT` / `--unset-default-provider` (each pair mutually exclusive). `default_provider` is the provider a NEW session in the project defaults to (inherited by sub-projects and git worktrees; unset = inherit).
...
- **New worktree:** `--worktree-branch BRANCH` creates the session in a brand-new git worktree of `--project` (the CLI counterpart of the UI's new-worktree button): `--project` becomes the source repo, the session lands in the worktree — registered as its own project linked back via `worktree_of`, inheriting the source's agent defaults/trust. `--worktree-path PATH` (absolute) is required with it; `--worktree-start-from REF` is optional (defaults to the source HEAD, ignored when the branch already exists). Shared orchestration with the HTTP endpoint via `core/services/worktree_creation.py`. Settings flags resolve against `--project`, which the worktree inherits from.
