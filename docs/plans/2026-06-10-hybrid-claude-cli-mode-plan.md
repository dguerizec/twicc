...
- [ ] **Step 4: Pending attributes plumbing — trusted-only**
...
- [ ] **Step 1: Add the helper** (pattern: `get_session_artifacts_dir`, line ~146)
...
- [ ] **Step 1: Hooks settings JSON — the single `PermissionRequest` hook + forced checkpointing**
...
- [ ] **Step 5: Manual check — full dry-run of the command line**
...
- [ ] **Step 3: Manager-side handlers** (in `manager.py`)
...
- [ ] **Step 4: JSONL state bridge in the sessions watcher**
...
- [ ] **Step 1: Hybrid settings categories** (`constants.py`, below `AGENT_SETTINGS_CATEGORIES`)
...
- [ ] **Step 3: Branch settings classification — including the mid-turn permission branch**
...
- [ ] **Step 4: Hybrid resume must never go through the SDK**
...
- [ ] **Step 5: Manual end-to-end (first real run)**
...
- [ ] **Step 1: `set_session_hybrid` handler** (pattern: neighboring session-update handlers in `WSConsumer`)
...
- [ ] **Step 2: Draft passthrough (explicit edit, not just a check)**
...
- [ ] **Step 2: Mount + suppression + no sending lock**
...
- [ ] **Step 3: Manual check (full loop in the browser)**
...
- [ ] **Step 1: Launch-side** — already done in Task 5 (`-n` = `Session.title` if set, else first 100 chars of the outgoing text). Verified effect: a custom title at launch permanently suppresses the CLI's own ai-title generation.

- [ ] **Step 2: Rename on title write**
...
- [ ] **Step 1: `adopt_running_hybrid_sessions()`**
...
- [ ] **Step 1:** For hybrid sessions, `getFieldHelpText` returns a short note on settings rows: "Applied on next message; some changes restart the CLI." Also surface the design-doc advisory once (e.g. a dismissible hint near the popover or in the terminal block): "Change settings here rather than inside the TUI — TwiCC does not read back TUI-side changes."

- [ ] **Step 2: Plug the existing stop affordances into the hybrid kill.** Today the Stop button, the menu entry, and the triple-Escape shortcut stop the SDK process via WS `kill_process` (`asgi.py:568`, frontend `useWebSocket.js:154`) → `manager.kill_agent` → `agent.interrupt_or_kill(reason)`. For hybrid sessions that chain must end in tmux+claude being killed: `HybridClaudeAgent.interrupt_or_kill` → `kill()` → `tmux kill-session` + DEAD broadcast (Task 5). Verify each of the three affordances end-to-end on a live hybrid session (works in any state, including mid-turn); fix the chain if any of them short-circuits on an SDK-only assumption.
...
- [ ] **Step 2: Original-file capture via file-history**
...
- [ ] **Step 3: Bump the compute version**
...
- [ ] **Step 1:** Add the feature entry under `[Unreleased]`.

- [ ] **Step 2: Full manual pass** (worktree servers, real browser):
