...
- **Change the session's settings or title:** `twicc update-session <session_id> settings ...` or `twicc update-session <session_id> title '<TEXT>'` — see the `twicc-update-session` skill. Archive / pin / stop will plug into the same sub-app later
