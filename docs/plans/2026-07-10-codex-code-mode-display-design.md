...
2. **MCP tier-1 rendering**: extractor already surfaces `mcp__*` calls; wiring them into the existing MCP result rendering is straightforward polish once the base lands.
3. **`world_state` / `turn_context`**: new 5.6 rollout item types, currently falling through generic classification. Out of scope here; worth a quick audit separately.
