...
3. **Concurrency & coalescing.** Prompts are serialized — one dialog at a time across hosts. Critically, a **burst of concurrent requests to the *same* host is coalesced into a single decision**: while a host's prompt is pending (or in-flight requests await it), every request to that host shares the one outcome (allow or deny) rather than each raising its own prompt. The decision is cleared once settled, so a *later* request re-evaluates fresh (an approved host hits the in-memory / persisted allowlist; a denied one re-asks). Without this, N parallel `fetch`es to one host would raise N prompts. *(Timed grants — "for 5 min / 1 h", stored via the value object's `expires_at`, §10 — are a deferred extension; v1 ships session + forever only.)*
