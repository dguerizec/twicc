...
- **`ArtifactBookmark`** — user bookmark of one rendered artifact (`session` FK, denormalised raw `project` FK, `relative_path`, `name`, `scope` reusing `PinMode`; unique `(session, relative_path)`). Scope mirrors session pinning (project/workspace/all, worktree-aware at resolution); "not bookmarked" = no row. Browsed via the sidebar's Artifacts mode (`/…/artifacts/:bookmarkId?`).
