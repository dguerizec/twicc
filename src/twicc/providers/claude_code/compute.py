...
class ClaudeCodeSessionCompute(BaseSessionCompute):
    ...
    Steps 2-3 wire the extraction surface, the live (watcher) machinery,
    and the batch (analyze_content + compute_session_metadata +
    apply_session_complete) onto this class. Each method delegates to a
    matching free function defined earlier in this file. The class is
    therefore stateless; :func:`get_compute` returns a per-process
    singleton.

    The only method still inherited from :class:`BaseSessionCompute` (and
    raising ``NotImplementedError``) is
    :meth:`sync_session_items_from_file`, which step 4 will migrate from
    sessions_watcher.py.
    """
