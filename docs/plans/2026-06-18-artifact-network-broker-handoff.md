...
- **Hardening — `window.parent` bypass (design §13): accepted residual risk in v1.** `allow-same-origin` lets a malicious artifact reach `window.parent` (bypassing the broker). The cheap fix (drop `allow-same-origin` → opaque origin + explicit-origin CSP) was **E2E-prototyped 2026-06-19 and works** (hole closed, assets + broker + authenticated API all survive) **but kills `localStorage`/`cookie`/IndexedDB** (opaque origin → `SecurityError`), so it was **reverted** — keep `allow-same-origin`. The real fix = a **separate real origin** for artifacts (keeps storage + closes the hole) but is a genuine project (separate-origin serving + broker two-origin rework), out of v1. See design §13 for the full write-up.
