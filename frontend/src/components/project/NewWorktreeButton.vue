<script setup>
// NewWorktreeButton.vue - Compact "create a worktree" button for the project
// rows of the "New session" dropdowns. Same structure and styling as the "…"
// actions button of the project selector rows (ProjectSelectorRow.vue): a
// plain wa-button inside a near-full-height overlay pinned to the right of
// the row. The click is stopped so the row's own selection (create a session
// in that project) does not fire.
...
<template>
    <span class="new-worktree-overlay" @click.stop>
        <wa-button
            appearance="plain"
            size="small"
            class="new-worktree-trigger"
            title="New worktree"
            @click="onClick"
        >
            <wa-icon name="code-branch" label="New worktree"></wa-icon>
...
.new-worktree-trigger::part(base) {
    padding: 0;
    min-height: 0;
    width: 100%;
...
.new-worktree-trigger::part(label) {
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 3px;
...
.new-worktree-trigger wa-icon {
    margin-inline: 0;
}

wa-dropdown-item:hover .new-worktree-trigger,
.new-worktree-overlay:focus-within .new-worktree-trigger {
    opacity: 1;
}
</style>
