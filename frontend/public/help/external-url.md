...
The **External URL** is where you reach *this* TwiCC instance from outside the
machine it runs on. TwiCC uses it to build links back to your sessions, so an
alert or a shared link opens the right conversation instead of a dead
`localhost` address.
...
External URL. A few popular options:
...
password.** So an External URL only actually works once you've run
