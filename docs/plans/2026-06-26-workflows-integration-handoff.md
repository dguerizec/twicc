...
**Rebased on `main`**; tip now `b95ed824` (*feat(workflow): surface orphaned runs as interrupted at read time*). Committed: everything in §2 — the data pipeline, renderer + enrichment, latency fix + Unassigned bucket, phase-state-on-the-back, CLI, run-tab shortcuts + palette, the nav fix, incomplete + interrupted runs, and orphaned-run detection. No migration anywhere — `phaseCompletion`/`statusKind`/`detectionUnavailable` all live inside `raw_json`; the orphan + detection-failure paths are **read-time derivations** (nothing stored), migrations `0116`/`0117`/`0118` were already applied. **Caveat:** repeated rebases on `main` rewrote every commit hash — find a commit by its message, not its sha. **Uncommitted** (held back on the user's call): the **phase-detection-failure** work (front `workflowTemplates.js` / `WorkflowsPane.vue` / `WorkflowRunDetail.vue` + back `views.py` / `workflow_synthesis.py`), this handoff, and the `CLAUDE.md` / `AGENTS.md` `Workflow` bullet. A backend restart is pending for the detection-failure endpoint/build changes (front is HMR; the old endpoint ignores the new flag, so no breakage meanwhile).
