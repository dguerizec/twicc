...
Update a project's name, color, archived state, default provider, and/or worktree directory — plus a `settings` sub-command for its per-provider agent-settings defaults. The directory is immutable; there is no delete (projects are archived, never removed).
