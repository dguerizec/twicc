...
- `generated/v2_all.py` — `SubAgentActivityKind`: added `completed = "completed"` (2026-08-27, on 0.150.1; still needed at 0.151.0). The runtime emits `subAgentActivity` items with that kind, but the generated models at the same tag lag behind; without the patch, `thread/resume` validation (`ThreadResumeResponse`) rejects any rollout containing one and the session can no longer resume.
